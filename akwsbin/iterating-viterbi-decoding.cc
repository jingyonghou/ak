...
int main(int argc, char *argv[]) {
    try {
        ...
        const char *usage = 
            "do hmm based kws without filler model and using iterating-biterbi-dicoding method to do kws\n"
            "Usage: iterating-viterbi-decoding <model-in> <graphs-rspecifier> <feature-rspecifier> <score-wspecifier> \n"
            "e.g.: \n"
            "iterating-viterbi-decoding 1.mdl graphs.fsts ark:test.scp ark,t:result.txt \n";
        ...
        std::string model_in_filename = po.GetArg(1),
            ...
            scores_wspecifier = po.GetOptArg(4);
        ...
        for (; !feature_reader.Done(); feature_reader.Next(), num_done++) {
            ...
            for (int i = 0; i < keyword_ids.size(); i++) {
                float score = akwspotter.SegmentationByFillerReestimation(fsts[i], &gmm_decodable);
